...
colnames(design) <- c("Missing", "StageI", "StageII", "StageIII", "StageIV", "sample_type")
...
my.contrasts <- makeContrasts(IvsII=StageI-StageII, IvsIII=StageI-StageIII, IvsIV=StageI-StageIV,
                                IIvsIII=StageII-StageIII, IIvsIV=StageII-StageIV, IIIvsIV=StageIII-StageIV, levels=design)

IvsII_lrt <- glmLRT(fit, contrast=my.contrasts[,"IvsII"])

IvsIII_lrt <- glmLRT(fit, contrast=my.contrasts[,"IvsIII"])

IvsIV_lrt <- glmLRT(fit, contrast=my.contrasts[,"IvsIV"])

IIvsIII_lrt <- glmLRT(fit, contrast=my.contrasts[,"IIvsIII"])

IIvsIV_lrt <- glmLRT(fit, contrast=my.contrasts[,"IIvsIV"])

IIIvsIV_lrt <- glmLRT(fit, contrast=my.contrasts[,"IIIvsIV"])
...
# IvsIV
topGenes_IvsIV <- topTags(IvsIV_lrt, adjust.method = "fdr", p.value = 0.05, n=Inf)
dim(topGenes_IvsIV)
#0 0
...
# IIvsIV
topGenes_IIvsIV <- topTags(IIvsIV_lrt, adjust.method = "fdr", p.value = 0.05, n=Inf)
dim(topGenes_IIvsIV)
#0 0

# IIIvsIV
topGenes_IIIvsIV <- topTags(IIIvsIV_lrt, adjust.method = "fdr", p.value = 0.05, n=Inf)
dim(topGenes_IIIvsIV)
#0 0

#Total number of genes: 150
...
# IvsIV
IvsIV_de.genes <- decideTestsDGE(IvsIV_lrt, adjust.method = "fdr", p.value = 0.05)
# get summary
summary(IvsIV_de.genes)
#       1*StageI -1*StageIV
#Down                     0
#NotSig               22649
#Up                       0
...
# IIvsIV
IIvsIV_de.genes <- decideTestsDGE(IIvsIV_lrt, adjust.method = "fdr", p.value = 0.05)
# get summary
summary(IIvsIV_de.genes)
#       1*StageII -1*StageIV
#Down                      0
#NotSig                22649
#Up                        0
...
# IIIvsIV
IIIvsIV_de.genes <- decideTestsDGE(IIIvsIV_lrt, adjust.method = "fdr", p.value = 0.05)
# get summary
summary(IIIvsIV_de.genes)
#       1*StageIII -1*StageIV
#Down                       0
#NotSig                 22649
#Up                         0
...
IvsIV_data <- topGenes_IvsIV$table
IIvsIII_data <- topGenes_IIvsIII$table
IIvsIV_data <- topGenes_IIvsIV$table
IIIvsIV_data <- topGenes_IIIvsIV$table

IvsII_data <- tibble::rownames_to_column(IvsII_data, var = "gene_id")
IvsIII_data <- tibble::rownames_to_column(IvsIII_data, var = "gene_id")
IvsIV_data <- tibble::rownames_to_column(IvsIV_data, var = "gene_id")
IIvsIII_data <- tibble::rownames_to_column(IIvsIII_data, var = "gene_id")
IIvsIV_data <- tibble::rownames_to_column(IIvsIV_data, var = "gene_id")
IIIvsIV_data <- tibble::rownames_to_column(IIIvsIV_data, var = "gene_id")
...
write.csv(IvsII, file ="C:/Users/Marion/Desktop/DGE/CA/Stage-CA/IvsII-DGE.csv", row.names = FALSE)

IvsIII <- IvsIII_data[order(IvsIII_data$logFC, decreasing = TRUE),]
write.csv(IvsIII, file ="C:/Users/Marion/Desktop/DGE/CA/Stage-CA/IvsIII-DGE.csv", row.names = FALSE)

IvsIV <- IvsIV_data[order(IvsIV_data$logFC, decreasing = TRUE),]
write.csv(IvsIV, file ="C:/Users/Marion/Desktop/DGE/CA/Stage-CA/IvsIV-DGE.csv", row.names = FALSE)

IIvsIII <- IIvsIII_data[order(IIvsIII_data$logFC, decreasing = TRUE),]
write.csv(IIvsIII, file ="C:/Users/Marion/Desktop/DGE/AA/Stage-AA/IIvsIII-DGE.csv", row.names = FALSE)

IIvsIV <- IIvsIV_data[order(IIvsIV_data$logFC, decreasing = TRUE),]
write.csv(IIvsIV, file ="C:/Users/Marion/Desktop/DGE/CA/Stage-CA/IIvsIV-DGE.csv", row.names = FALSE)

IIIvsIV <- IIIvsIV_data[order(IIIvsIV_data$logFC, decreasing = TRUE),]
write.csv(IIIvsIV, file ="C:/Users/Marion/Desktop/DGE/CA/Stage-CA/IIIvsIV-DGE.csv", row.names = FALSE)
...
significance = -log10(topGenes_IvsIII$table$FDR) < 1
topGenes_IvsIII$table$significance <- significance
dim(topGenes_IvsIII$table)

## setting the values
topGenes_IvsIII$table$diffexpressed <- "No"

# if logFC > 0.1 and FDR < 0.05, set as "UP"
topGenes_IvsIII$table$diffexpressed[topGenes_IvsIII$table$logFC > 1 & topGenes_IvsIII$table$FDR < 0.05] <- "Up"

# if logFC < -1 and pvalue < 0.05, set as "DOWN"
topGenes_IvsIII$table$diffexpressed[topGenes_IvsIII$table$logFC < -1 & topGenes_IvsIII$table$FDR < 0.05] <- "Down"
...
input <- cbind(gene=rownames(topGenes_IvsIII$table), topGenes_IvsIII$table) #convert the rownames to a column
volc = ggplot(input, aes(logFC, -log10(PValue))) + #volcanoplot with logFC versus pvalue
  ...
  ggtitle("Stage I vs Stage III CA patients")
...
selY <- y[rownames(topGenes_IIvsIII$table)[topGenes_IIvsIII$table$FDR<0.05 & 
                                             abs(topGenes_IIvsIII$table$logFC)>1.5],]
