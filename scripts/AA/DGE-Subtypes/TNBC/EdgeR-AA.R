...
directory <- "C:/Users/Marion/Desktop/DGE/AA/all-counts/TNBC/"
dir(directory)

# read files
rawCountTable <- read.csv(paste0(directory,"reads_count.csv"), header=TRUE,
                            row.names=1)
sampleInfo <- read.csv(paste0(directory,"matadata-TNBC.csv"), header=FALSE,
                         row.names=1)
...
condition = c("Primary Tumor", "Primary Tumor", "Primary Tumor", "Primary Tumor", 
              "Primary Tumor", "Primary Tumor", "Solid Tissue Normal", 
              "Primary Tumor", "Solid Tissue Normal", "Solid Tissue Normal", "Solid Tissue Normal", 
...
write.csv(sigDownReg, file="C:/Users/Marion/Desktop/DGE/AA/all-counts/TNBC/sigDownReg-filtered.csv")
write.csv(sigUpReg, file="C:/Users/Marion/Desktop/DGE/AA/all-counts/TNBC/sigUpReg-filtered.csv")
...
volc = ggplot(input, aes(logFC, -log10(PValue))) + #volcanoplot with logFC versus pvalue
  geom_point() +
  theme_classic()+
  geom_vline(xintercept=c(-1.5, 2), col="orange", linetype="dashed") +
  geom_hline(yintercept=-log10(0.001), col="orange" , linetype="dashed")+
