...
colnames(design) <- c("StageI", "StageII", "StageIII", "StageIV", "sample_type")
...
write.csv(IvsII, file ="C:/Users/Marion/Desktop/DGE/AA/Stage-AA/IvsII-DGE.csv", row.names = FALSE)

IvsIII <- IvsIII_data[order(IvsIII_data$logFC, decreasing = TRUE),]
write.csv(IvsIII, file ="C:/Users/Marion/Desktop/DGE/AA/Stage-AA/IvsIII-DGE.csv", row.names = FALSE)

IvsIV <- IvsIV_data[order(IvsIV_data$logFC, decreasing = TRUE),]
write.csv(IvsIV, file ="C:/Users/Marion/Desktop/DGE/AA/Stage-AA/IvsIV-DGE.csv", row.names = FALSE)
...
write.csv(IIvsIV, file ="C:/Users/Marion/Desktop/DGE/AA/Stage-AA/IIvsIV-DGE.csv", row.names = FALSE)

IIIvsIV <- IIIvsIV_data[order(IIIvsIV_data$logFC, decreasing = TRUE),]
write.csv(IIIvsIV, file ="C:/Users/Marion/Desktop/DGE/AA/Stage-AA/IIIvsIV-DGE.csv", row.names = FALSE)
...
significance = -log10(topGenes_IIIvsIV$table$FDR) < 1
topGenes_IIIvsIV$table$significance <- significance
dim(topGenes_IIIvsIV$table)

## setting the values
topGenes_IIIvsIV$table$diffexpressed <- "No"

# if logFC > 0.1 and FDR < 0.05, set as "UP"
topGenes_IIIvsIV$table$diffexpressed[topGenes_IIIvsIV$table$logFC > 1 & topGenes_IIIvsIV$table$FDR < 0.05] <- "Up"

# if logFC < -1 and pvalue < 0.05, set as "DOWN"
topGenes_IIIvsIV$table$diffexpressed[topGenes_IIIvsIV$table$logFC < -1 & topGenes_IIIvsIV$table$FDR < 0.05] <- "Down"
...
input <- cbind(gene=rownames(topGenes_IIIvsIV$table), topGenes_IIIvsIV$table) #convert the rownames to a column
volc = ggplot(input, aes(logFC, -log10(PValue))) + #volcanoplot with logFC versus pvalue
  ...
  ggtitle("Stage III vs Stage IV AA patients")
...
selY <- y[rownames(topGenes_IIIvsIV$table)[topGenes_IIIvsIV$table$FDR<0.01 & 
                                             abs(topGenes_IIIvsIV$table$logFC)>1.5],]
